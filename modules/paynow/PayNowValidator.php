...
class PayNowValidator {
    ...
    private function getHost($which = '') {
        switch( $which ) {
            case 'auth':
                $host = "https://mobi.netcash.co.za/"; // authenticate.aspx?key={SagePayUsername}&message={Encrypted xml message}
                if( $this->debugging  ) {
                    $host = "http://mobi.staging.netcash.co.za:7999/"; // authenticate.aspx?key={SagePayUsername}&message={Encrypted xml message}
                }
                break;
